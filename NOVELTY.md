...
| Transmission-aware HSV edge bands | Laplacian/Gaussian dehazing, wavelet dehazing, Domain Transform и bilateral dehazing уже известны | одна физическая часть и transmission/scale gate, но residual bands строятся по HSV-V без фильтрации Hue/Saturation | 2×2 val; frozen O-HAZE test; 54 tests; scene-08 review | Узкий quality-кандидат. Не заявлять новым сам basis или speedup; на test SSIM лучше baseline на 21/22, но 800 px timing примерно на 13% медленнее |
...
| .NET-лаборатория из 54 методов с единым API | аналогов немного | воспроизводимый стенд и контролируемое сравнение | бенчмарк | **Самый защищаемый вклад репозитория** |
