...
\hypersetup{
  ...
  pdftitle={Uncertainty-Aware Airlight-Aligned Dual-Gain Recovery with Exact RGB Feasibility},
  pdfauthor={Maksim Sitnikov},
  pdfsubject={Training-free single-image dehazing},
  pdfkeywords={image dehazing, uncertainty, RGB feasibility, total variation}
}
...
recovery framework, not state-of-the-art dehazing. Code, tests, manifests, and statistical
artifacts accompany the paper.
\end{abstract}
...
raw CSV files, 10,000-bootstrap outputs, and 43 deterministic unit tests. It also distinguishes
...
improvements in structure, noise, and feasibility. Mixed real-paired results and the CAR case study
show why those gains must not be generalized into a universal color or perceptual claim. The next
decisive experiment is a pre-registered blind external evaluation and comparison with complete
