...
## Дополнение: HSV-V, edge-aware bands и UTAW
...
После UTAW follow-up в GUI сохранены все три варианта, но в curated-набор входят старый
Laplacian и HSV UTAW; HSV Edge остаётся сравнительной абляцией. C³R-HSV и HSV²CR туда не
возвращаются: код сохраняется для воспроизводимости абляций, но
...
### Clean UTAW follow-up

Validation-selected U1 запущен на clean commit `bd4660a`, `--maxdim=800`. Сцена и её GT уже
просматривались при разработке, поэтому таблица является development case, а не blind test.
...
UTAW существенно поднял SSIM и убрал часть блочной/мозаичной фактуры Edge. При этом он не
получил нового зафиксированного авторского ранга: прежнее предпочтение старого Transmission-aware
остаётся действительным до нового просмотра или blind pairwise study.

Отдельный HCV-контроль на этой сцене также отрицателен: A²CR получил примерно
`15.738/0.664/14.736`, HCV-A²CR `15.489/0.660/14.911`, HCV-A²CR-UTAW
`15.441/0.655/15.031` по full PSNR/SSIM/DE00. HCV-ветвь не заменяет A²CR.
