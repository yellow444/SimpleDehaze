...
**Статус:** экспериментальный метод, зарегистрирован в GUI и `--selftest`; в статье используется
как дополняющая отрицательная абляция, а не как самостоятельная SOTA-заявка.
...
value clipping, а не более высокое качество удаления дымки. Поэтому рекомендуемый формат — общая
статья A²CR+C³R, где A²CR остаётся основным методом, а C³R служит комплементарной геометрией и
отрицательной абляцией.
